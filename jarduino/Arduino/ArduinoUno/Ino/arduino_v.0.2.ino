...
void startPumpFor(uint16_t seconds) {
  ...
  if (!pumpActive) {
    ...
    Serial.print("Bomba activada por "); Serial.print(seconds); Serial.println("s");
  }
}
...
void checkWiFi() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    WiFi.begin(WIFI_SSID, WIFI_PASS);
    // No usamos delay prolongado, solo un intento rápido
  }
}
...
void loop() {
  ...
  // Leer sensores cada 5s (o tu intervalo configurado)
